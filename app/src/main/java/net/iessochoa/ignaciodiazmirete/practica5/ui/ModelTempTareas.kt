...
object ModelTempTareas {
    ...
    operator fun invoke(context: Context){
     ...
     //devuelve un LiveData en vez de MutableLiveData para evitar su modificación en las capas superiores
    fun getAllTareas(): LiveData<List<Tarea>> {
        tareasLiveData.value= tareas
        return tareasLiveData
    }
     //añade una tarea, si existe (id iguales) la sustituye y si no la añade. Posteriormente actualiza el LiveData que permitirá avisar a quien esté observando
    fun addTarea(tarea: Tarea) {
     ...
     //Borra una tarea y actualiza el LiveData para avisar a los observadores
    ...
    fun getTareasFiltroSinPagar(soloSinPagar:Boolean): LiveData<List<Tarea>> {
        ...
        tareasLiveData.value= if(soloSinPagar) tareas.filter { !it.pagado } as ArrayList<Tarea>
        else
            tareas
        return tareasLiveData
    }
